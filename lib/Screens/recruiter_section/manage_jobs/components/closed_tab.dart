...
import 'manage_jobs_dialog.dart';
...
class _ClosedTabState extends State<ClosedTab> {
  ...
  Future loaddata() async {
    await managejobcontrol.getclosejob();
    if(mounted){
      setState(() {
      loading = false;
    });
    ...
    return loading
        ...
        : SingleChildScrollView(
            child: Container(
              ...
              children: [
                Obx((){
                  return managejobcontrol.closedJoblist.isEmpty 
                  ?Center(child: EmptyJobPostTile(text: "There are no closed jobs")) :
                Align(
                ...
                child: ListView.builder(
                      ...
                      itemCount: managejobcontrol.closedJoblist.length,
                      itemBuilder: (_, index) {
                        var data = managejobcontrol.closedJoblist[index];
                        return JobsOpeningTile(
                          onTap: () {
                            Get.put(JobPostPreviewController())
                                .getJobPreviewData(jobId: data.id);
                            Get.to(
                                JobPostPreviewPage(managejob: true, tabindex: 2,jobid: data.id));
                          },
                          isRespotBtn: true,
                          isCrossBtn: true,
                          onRepostPressed: ()=> managejobcontrol.repost_closejob(jobid: data.id!,fields:{"job_status_type" : 1}),
                          onClosePressed: () => ManageJobDialog.dialog(isCloseJob: false,jobId: data.id!),
