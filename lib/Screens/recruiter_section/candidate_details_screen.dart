...
class CandidateDetailsScreen extends StatefulWidget {
  ...
  final Candidatelist? index;
  CandidateDetailsScreen({super.key, this.isArrowIcon = false, this.index});
...
class _CandidateDetailsScreenState extends State<CandidateDetailsScreen> {
  ...
  Future interchatinbox() async {
    ...
    //           builder: (context) => RecruiterChatScreen(data: channeldata)));
    // }
    if (premiunmessage(channeldata, chatControll,
                ...
                InkResponse(
                    onTap: () async {
                      var result = await Share.shareWithResult(
                          'https://bringin.io/candidatedetails/${widget.index!.id}');
                ...
                InkResponse(
                    onTap: () {
                      Get.toNamed(RouteHelper.getReportRoute(), arguments: {
                        "seekerId": widget.index!.userid!.id!,
                        "full_detail_id": widget.index!.id,
                      });
                    },
                    child: Padding(
                      padding: const EdgeInsets.all(8.0),
                  ...
                  child: Wrap(
                    ...
                    children: [
                      ...List.generate(
                        ...
                        (index) {
                          return MySkillsTile(
                                  indicatorColor: widget.index!.skill!.length - 1 ==
                                                    index
                                                ? Colors.transparent
