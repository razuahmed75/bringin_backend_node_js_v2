...
import '../messages/candidate_message/components/chats_dialog.dart';
import '../messages/recruiter_message/Inbox/inbox.dart';

class ViewCandidateDetailsScreen extends StatefulWidget {
  final bool? isArrowIcon;
  final Candidatefullprofile? index;
  ViewCandidateDetailsScreen({super.key, this.isArrowIcon = false, this.index});

  @override
  State<ViewCandidateDetailsScreen> createState() =>
      _ViewCandidateDetailsScreenState();
}

class _ViewCandidateDetailsScreenState
    extends State<ViewCandidateDetailsScreen> {
  ...
  Future interchatinbox() async {
    ...
    var channeldata = await chatControll.channelcreate(
        widget.index!.userid!.id!, chatControll.currentuserid, null, widget.index!.id, false);

    if (chatControll.recruiter_today_conv.length >= 3) {
      if (chatControll.recruiter_today_conv
          .any((element) => element.id == channeldata.id)) {
        Navigator.push(
            context,
            MaterialPageRoute(
                builder: (context) => RecruiterChatScreen(data: channeldata)));
      } else {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      bottomNavigationBar: BottomNavWidget(
        text: loading ? "Start Chatting..." : "Start Chatting",
        onTap: loading ? null : () {
          interchatinbox();
        },
      ),
              ...
              : ListView.builder(
                  ...
                  itemBuilder: (_, index) {
                    ...
                    return GestureDetector(
                      onTap: () async{
                            try{
                              if(data.protfoliolink!.contains("http://") || data.protfoliolink!.contains("https://")){
                                await launchUrl(Uri.parse(data.protfoliolink!));
                              }else{
                                await launchUrl(Uri.parse("https://" + data.protfoliolink!));
                              }
                            }catch(e){
    ...
    return Container(
      ...
      child: Row(
        children: [
          GestureDetector(
            onTap: (){
              Get.to(()=>Scaffold(body: Center(
                child: PhotoView(
                  maxScale: 1.0,
                  minScale: 0.1,
                  imageProvider: NetworkImage(widget.index!.userid!.image == null
                  ...
                  child: ListView.builder(
                      ...
                      itemBuilder: (BuildContext context, index) {
                        ...
                        return CareerPreferenceWidget(
                            ...
                            isArrowIcon: false,
                            industryList: data.category!.isEmpty || data.category == null 
                            ? []
                            :List.generate(
