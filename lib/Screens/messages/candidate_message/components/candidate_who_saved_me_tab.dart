...
class CandidateWhoSavedMeTab extends StatefulWidget {
  const CandidateWhoSavedMeTab({super.key});

  @override
  State<CandidateWhoSavedMeTab> createState() => _CandidateWhoSavedMeTabState();
}

class _CandidateWhoSavedMeTabState extends State<CandidateWhoSavedMeTab> {
  ...
  load()async{
    ...
    await JobControll.to.getWhoSavedMe();
    if(mounted){
      setState(() {
      isLoading = false;
    });
  ...
  Widget build(BuildContext context) {
    ...
    ) : JobControll.to.whoSavedMeList.isEmpty 
    ? EmptyWhoSavedMe(
      onTap: ()=>_bottomNavController.goToInitialPage(),
      icon: AppImagePaths.emptySavedMe,
      description: AppStrings.whoSavedMeDes,
    ) 
    : ListView.builder(
      itemCount: JobControll.to.whoSavedMeList.length,
      itemBuilder: (_,index){
      var data = JobControll.to.whoSavedMeList[index];
      bool isNewVisible = !hiddenIndices!.containsKey(data!.id);
        return InkWell(
          onTap: () async{
            await JobControll.to.jobViewCount(fields: {
                    "jobid": data.id,
                    ...
                    child: JobsTile(
                      ...
                      child: Row(
                        ...
                        children: [
                          ...
                          if (isNewVisible) Container(
                            padding: EdgeInsets.symmetric(horizontal: width(4),vertical: 2),
                            decoration: BoxDecoration(
                              color: AppColors.newBtnColor,
                              borderRadius: BorderRadius.circular(12),
                            ),
                            child: Text("New",style: Styles.smallText.copyWith(
