...
class ViewedJobsScreen extends StatelessWidget {
  const ViewedJobsScreen({super.key});

  @override
  Widget build(BuildContext context) {
    JobControll.to.getViewedJobs();
    JobControll jobControll = Get.find();
    return Scaffold(
      appBar: appBarWidget(
          title: "Viewed Jobs", onBackPressed: () => Get.back(), actions: []),
      body: SafeArea(
        child: Padding(
            ...
            children: [
              ...
              Obx(
                () => jobControll.isLoading.value
                    ? Helpers.appLoader()
                    : jobControll.viewedJobList.isEmpty
                        ? Container(
                            height: Dimensions.screenHeight * .5,
                            width: double.maxFinite,
                            child: Center(
                              child: Text("No viewed jobs found"),
                            ),
                          )
                        : Expanded(
                            child: Align(
                              ...
                              child: ListView.builder(
                                shrinkWrap: true,
                                reverse: true,
                                  itemCount: jobControll.viewedJobList.length,
                                  itemBuilder: (_, index) {
                                    var data =
                                        jobControll.viewedJobList[index];
                                    return InkWell(
                                      onTap: () {
